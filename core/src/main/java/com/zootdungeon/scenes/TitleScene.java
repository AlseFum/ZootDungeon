...
import com.zootdungeon.windows.WndSettings;
...
public class TitleScene extends PixelScene {
	...
	public void create() {
		...
		Music.INSTANCE.playTracks(
				new String[]{Assets.Music.AK_SYS_VOID_INTRO},
				new float[]{1},
				false);
		...
		Image title = BannerSprites.get(BannerSprites.Type.CLOSURE_TITLE);
		add( title );
		...
		StyledButton btnRankings = new StyledButton(GREY_TR,Messages.get(this, "rankings")){
			@Override
			protected void onClick() {
				ColaDungeon.switchNoFade( RankingsScene.class );
			}
		};
		...
		StyledButton btnBadges = new StyledButton(GREY_TR, Messages.get(this, "journal")){
			@Override
			protected void onClick() {
				ColaDungeon.switchNoFade( JournalScene.class );
			}
		};
		...
		StyledButton btnAbout = new StyledButton(GREY_TR, Messages.get(this, "about")){
			@Override
			protected void onClick() {
				ColaDungeon.switchScene( AboutScene.class );
			}
		};
	...
	private static class NewsButton extends StyledButton {
		...
		public NewsButton(Chrome.Type type, String label ){
			super(type, label);
			if (CDSettings.news()) News.checkForNews();
		}
		...
		public void update() {
			...
			if (unreadCount == -1 && News.articlesAvailable()){
				long lastRead = CDSettings.newsLastRead();
				if (lastRead == 0){
					if (News.articles().get(0) != null) {
						CDSettings.newsLastRead(News.articles().get(0).date.getTime());
					}
				} else {
					unreadCount = News.unreadArticles(new Date(CDSettings.newsLastRead()));
					if (unreadCount > 0) {
		...
		protected void onClick() {
			super.onClick();
			ColaDungeon.switchNoFade( NewsScene.class );
		}
	}
	...
	private static class SettingsButton extends StyledButton {
		...
		protected void onClick() {
			...
			ColaDungeon.scene().add(new WndSettings());
		}
	}
	...
	private static class SupportButton extends StyledButton{
		...
		@Override
		protected void onClick() {
			ColaDungeon.switchNoFade(SupporterScene.class);
		}
	}
}
