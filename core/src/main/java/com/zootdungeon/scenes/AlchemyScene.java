...
import com.zootdungeon.ui.ItemSlot;
import com.zootdungeon.ui.RadialMenu;
...
import com.zootdungeon.windows.WndJournal;
...
public class AlchemyScene extends PixelScene {
						...
						if (Dungeon.hero != null) {
							...
							Game.scene().addToFront(new RadialMenu(Messages.get(Toolbar.class, "container_prompt"), info, names, images){
								...
								public void onSelect(int idx, boolean alt) {
									...
									if (items.size() == 0){
										ColaDungeon.scene().addToFront(new WndMessage(Messages.get(AlchemyScene.class, "no_items")));
										return;
									}
		...
		for (int i = 0; i < recipes.size(); i++){
			...
			if (!canAfford && Dungeon.hero != null) {
				...
				canAfford = Dungeon.hero.pointsInTalent(Talent.RESERVED_OP_ALCHEMY_SUBSIDY) > 0
						&& RhodesIslandTerminal.ownedBy(Dungeon.hero) != null
						&& cover <= RhodesIslandTerminal.costOrZero(Dungeon.hero);
			}
	...
	private void combine( int slot ){
		...
		if (recipe != null){
			...
			if (shortfall > 0) {
				RhodesIslandTerminal terminal = RhodesIslandTerminal.ownedBy(Dungeon.hero);
				if (terminal != null) {
					terminal.spendCost(Talent.reservedOpAlchemySubsidyCost(Dungeon.hero, shortfall));
				}
			}
