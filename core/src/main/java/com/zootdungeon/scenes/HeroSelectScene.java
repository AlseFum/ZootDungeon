...
import com.zootdungeon.ui.ExitButton;
...
import com.zootdungeon.utils.DungeonSeed;
import com.zootdungeon.windows.WndChallenges;
import com.zootdungeon.windows.WndHeroInfo;
...
public class HeroSelectScene extends PixelScene {
	...
	public void create() {
		...
		infoButton = new IconButton(Icons.get(Icons.INFO)){
			...
			protected void onClick() {
				...
				if (cls != null) {
					...
					ColaDungeon.scene().addToFront(w);
				}
			}
		...
		if (!Badges.isUnlocked(Badges.Badge.VICTORY) && !DeviceCompat.isDebug()){
			...
			CDSettings.challenges(0);
			CDSettings.customSeed("");
		}
		...
		add( btnExit );
		btnExit.visible = btnExit.active = !CDSettings.intro();
	...
	private void updateOptionsColor(){
		if (!CDSettings.customSeed().isEmpty()){
			btnOptions.icon().hardlight(1f, 1.5f, 0.67f);
		} else if (CDSettings.challenges() != 0){
	...
	private void setSelectedHero(HeroClass cl){
		...
		if (landscape()) {
			...
			btnOptions.visible = btnOptions.active = !CDSettings.intro();
	...
	public void update() {
		super.update();
		if (CDSettings.intro() && Rankings.INSTANCE.totalNumber > 0){
	...
	private class HeroBtn extends StyledButton {
		...
		protected void onClick() {
			...
			if( !cl.isUnlocked() ){
				ColaDungeon.scene().addToFront( new WndMessage(cl.unlockMsg()));
		...
		protected void createChildren() {
			...
			StyledButton seedButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "custom_seed"), 6){
				...
				protected void onClick() {
					...
					String existingSeedtext = CDSettings.customSeed();
					ColaDungeon.scene().addToFront(new WndTextInput(Messages.get(HeroSelectScene.class, "custom_seed_title"),
							...
							if (positive && seed != -1){
								...
								for (GamesInProgress.Info info : GamesInProgress.checkAll()){
									if (info.customSeed.isEmpty() && info.seed == seed){
										CDSettings.customSeed("");
										icon.resetColor();
										ColaDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "custom_seed_duplicate")));
										return;
									}
								}

								CDSettings.customSeed(text);
			...
			if (!CDSettings.customSeed().isEmpty()) seedButton.icon().hardlight(1f, 1.5f, 0.67f);;
			...
			StyledButton dailyButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "daily"), 6){
				...
				protected void onClick() {
					...
					long diff = (CDSettings.lastDaily() + DAY) - Game.realTime;
					if (diff > 24*HOUR){
						ColaDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_unavailable_long", (diff / DAY)+1)));
						return;
					}

					for (GamesInProgress.Info game : GamesInProgress.checkAll()){
						if (game.daily){
							ColaDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_existing")));
							return;
						}
					}
					...
					ColaDungeon.scene().addToFront(new WndOptions(
							icon,
							Messages.get(HeroSelectScene.class, "daily"),
							diff > 0 ?
				...
				protected void onClick() {
					...
					ColaDungeon.scene().addToFront(new WndChallenges(CDSettings.challenges(), true) {
						public void onBackPressed() {
							super.onBackPressed();
							icon(Icons.get(CDSettings.challenges() > 0 ? Icons.CHALLENGE_COLOR : Icons.CHALLENGE_GREY));
							updateOptionsColor();
						}
					} );
				}
			...
			challengeButton.icon(Icons.get(CDSettings.challenges() > 0 ? Icons.CHALLENGE_COLOR : Icons.CHALLENGE_GREY));
			add(challengeButton);
