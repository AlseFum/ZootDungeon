...
import com.zootdungeon.ui.Tooltip;
import com.zootdungeon.ui.Window;
import com.zootdungeon.utils.Holiday;
...
public class PixelScene extends Scene {
	...
	public void create() {
		...
		Game.platform.setupFontGenerators(renderedTextPageSize, CDSettings.systemFont());
	...
	public void update() {
		...
		if (DeviceCompat.isDesktop() && fullscreenListener == null){
			KeyEvent.addKeyListener(fullscreenListener = new Signal.Listener<KeyEvent>() {
				...
				public boolean onSignal(KeyEvent keyEvent) {
					...
					if (alt && enter){
						CDSettings.fullscreen(!CDSettings.fullscreen());
						return true;
					}
		...
		if (!Cursor.isCursorCaptured()) {
			if (Math.abs(ControllerHandler.rightStickPosition.x) >= 0.2f
				...
				int sensitivity = CDSettings.controllerPointerSensitivity() * 100;
	...
	public static boolean landscape(){
		return CDSettings.interfaceSize() > 0 || Game.width > Game.height;
	}
	...
	public static void shake( float magnitude, float duration){
		magnitude *= CDSettings.screenShake();
