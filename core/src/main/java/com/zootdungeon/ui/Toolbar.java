...
import com.zootdungeon.windows.WndQuickBag;
import com.zootdungeon.windows.WndUseItem;
...
public class Toolbar extends Component {
	...
	protected void createChildren() {
		...
		add(btnInventory = new Tool(0, 0, 24, 26) {
			...
			protected void createChildren() {
				...
				arrow.visible = CDSettings.interfaceSize() == 2;
	...
	protected void layout() {
		...
		int startingSlot;
		if (CDSettings.quickSwapper() && quickslotsToShow < 6){
			quickslotsToShow = 3;
		...
		for(int i = startingSlot; i <= endingSlot; i++) {
			if (i == startingSlot && !CDSettings.flipToolbar() ||
				i == endingSlot && CDSettings.flipToolbar()){
				btnQuick[i].border(0, 2);
				btnQuick[i].frame(106, 0, 19, 24);
			} else if (i == startingSlot && CDSettings.flipToolbar() ||
					i == endingSlot && !CDSettings.flipToolbar()){
				btnQuick[i].border(2, 1);
				btnQuick[i].frame(86, 0, 20, 24);
			} else {
		...
		Toolbar.Mode mode;
		try {
			mode = Mode.valueOf(CDSettings.toolbarMode());
		} catch (Exception e){
