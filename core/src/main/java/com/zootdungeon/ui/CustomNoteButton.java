...
import com.zootdungeon.windows.WndJournalItem;
					...
					GameScene.show(new WndTextInput(Messages.get(CustomNoteWindow.class, "edit_title"),
						...
						public void onSelect(boolean positive, String text) {
							if (positive && !text.isEmpty()){
								...
								ColaDungeon.scene().addToFront(new CustomNoteWindow(rec));
							}
						}
					});
				}
					...
					GameScene.show(new WndOptions(Icons.WARNING.get(),
						...
						protected void onSelect(int index) {
							if (index == 0){
								Notes.remove(rec);
								CustomNoteWindow.this.hide();
								ColaDungeon.scene().addToFront(new WndJournal());
							}
						}
					});
				}
		...
		GameScene.show(new WndTextInput(promptTitle,
			...
			public void onSelect(boolean positive, String text) {
				if (positive && !text.isEmpty()){
					...
					hide();
					ColaDungeon.scene().addToFront(new WndJournal());
					ColaDungeon.scene().addToFront(new CustomNoteWindow(note));
				}
			}
		});
	}
