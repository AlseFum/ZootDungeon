...
package com.zootdungeon.items.trinkets;
...
import com.zootdungeon.ui.ItemButton;
...
import com.zootdungeon.windows.WndSadGhost;
...
public class TrinketCatalyst extends Item {
	...
	public static class Recipe extends com.zootdungeon.items.Recipe {
		...
		public Item brew(ArrayList<Item> ingredients) {
			...
			ColaDungeon.scene().addToFront(new WndTrinket(newCata));
			try {
			...
			for (int i = 0; i < NUM_TRINKETS; i++){
				ItemButton btnReward = new ItemButton() {
					...
					protected void onClick() {
						ColaDungeon.scene().addToFront(new RewardWindow(item()));
					}
				};
				if (i == NUM_TRINKETS-1){
					...
					protected void onClick() {
						...
						if (cata != null) {
							...
							if (ColaDungeon.scene() instanceof AlchemyScene) {
								((AlchemyScene) ColaDungeon.scene()).craftItem(null, result);
							} else {
