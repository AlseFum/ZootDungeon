...
public class SummoningThrowingWeapon extends MissileWeapon {
    ...
    @Override
    public String desc() {
        String desc = "一把特殊的投掷武器。";
        desc += "\n\n投掷时会对目标造成伤害。如果伤害致死敌人，会在敌人位置生成一个仆役。";
        desc += "\n\n仆役会追随玩家，并攻击玩家一定范围内的敌人。";
        desc += "\n\n仆役的强度与武器等级相关。";
        return desc;
    }
    ...
    public static class SummonedServant extends Mob {
        ...
        protected boolean act() {
            ...
            if (target != null) {
                ...
                // 没有敌人，追随玩家
                // 保持 HUNTING 状态，设置 target 为玩家位置，这样会跟随玩家
                enemy = null;
                enemySeen = false;
                state = HUNTING;
        ...
        private void followHero() {
            ...
            if (distToHero > FOLLOW_RANGE) {
                // 使用 getCloser 方法移动到玩家附近
                // getCloser 已经处理了移动和时间消耗
