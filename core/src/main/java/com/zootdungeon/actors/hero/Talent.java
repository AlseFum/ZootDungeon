...
public class Talent implements Bundlable {
	...
	public static void onFoodEaten( Hero hero, float foodVal, Item foodSource ){
		RhodesIslandTerminal terminal = (RhodesIslandTerminal) hero.belongings.getItem(RhodesIslandTerminal.class);
		if (terminal == null) return;
		if (hero.pointsInTalent(RESERVED_OP_FIELD_RATION) > 0) {
			int gain = 2 + 2 * hero.pointsInTalent(RESERVED_OP_FIELD_RATION);
			int actual = terminal.addCost(hero, gain);
			if (actual > 0) {
				GLog.p(Messages.get(Talent.class, "reserved_op_field_ration.msg", actual));
			}
		}
		if (hero.pointsInTalent(RESERVED_OP_RAPID_MEAL) > 0) {
			...
			int actual = terminal.addCost(hero, gain);
			if (actual > 0) {
				GLog.p(Messages.get(Talent.class, "reserved_op_rapid_meal.msg", actual));
			}
		}
	}
