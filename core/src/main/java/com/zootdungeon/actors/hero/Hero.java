...
import com.zootdungeon.actors.buffs.TimeStasis;
...
import com.zootdungeon.actors.hero.spells.HallowedGround;
import com.zootdungeon.actors.hero.spells.HolyWard;
...
import com.zootdungeon.items.artifacts.CapeOfThorns;
...
import com.zootdungeon.levels.rooms.special.WeakFloorRoom;
...
public class Hero extends Char {
    ...
    public Hero(HeroClass _heroClass) {
        super();
    ...
    public int damageRoll() {
        ...
        AttackUpBuff atkUp = buff(AttackUpBuff.class);
        if (atkUp != null) {
            dmg = Math.round(dmg * atkUp.multiplier);
        }
    ...
    private boolean actAlchemy(HeroAction.Alchemy action) {
        ...
        if (Dungeon.level.distance(dst, pos) <= 1) {
            ...
            ColaDungeon.switchScene(AlchemyScene.class);
            return false;
                        ...
                        protected boolean act() {
                            if (enemy.isAlive()) {
                                if (hasTalent(Talent.SHARED_UPGRADES)) {
                                    ...
                                    float bonusDmg = wep.buffedLvl() * ((MissileWeapon) wep).tier
                                            * pointsInTalent(Talent.SHARED_UPGRADES) * 0.025f;
    ...
    // FIXME this is a fairly crude way to track this, really it would be nice to
    // have a short
    ...
    public boolean handle(int cell) {
        ...
        } else if (fieldOfView[cell] && ch instanceof MapDevice) {

            // 关卡地图装置：占格、可被攻击；走 Attack 触发 MapDevice.receiveDamage(src)
            // （MapDevice.damage 重写为 redirect 到该入口，不走 Char.damage 链扣 HP）。
