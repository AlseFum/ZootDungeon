...
import com.zootdungeon.ui.TalentButton;
import com.zootdungeon.ui.TalentsPane;
		...
		public StatsTab() {
			...
			if (Dungeon.hero == null){
				...
				IconButton scoreInfo = new IconButton(Icons.get(Icons.INFO)) {
					...
					protected void onClick() {
						super.onClick();
						ColaDungeon.scene().addToFront(new WndScoreBreakdown());
					}
				};
			...
			if (Dungeon.hero != null && Dungeon.seed != -1 && !Dungeon.daily &&
				...
				RedButton btnSeed = new RedButton(Messages.get(this, "copy_seed")){
					...
					protected void onClick() {
						super.onClick();
						ColaDungeon.scene().addToFront(new WndOptions(new Image(icon),
							...
							protected void onSelect(int index) {
								...
								if (index == 0){
									CDSettings.customSeed(DungeonSeed.convertToCode(Dungeon.seed));
				...
				if (DungeonSeed.convertFromText(CDSettings.customSeed()) == Dungeon.seed){
					icon.hardlight(1f, 1.5f, 0.67f);
				}
				btnSeed.icon(icon);
