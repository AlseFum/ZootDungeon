...
public class WndEnergizeItem extends WndInfoItem {
	...
	private static void energize(Item item){
		...
		if (ColaDungeon.scene() instanceof AlchemyScene){
			...
			((AlchemyScene) ColaDungeon.scene()).createEnergy();
			if (!item.isIdentified()){
				((AlchemyScene) ColaDungeon.scene()).showIdentify(item);
			}
	...
	public static WndBag openItemSelector(){
		if (ColaDungeon.scene() instanceof GameScene) {
			...
			ColaDungeon.scene().addToFront(window);
			return window;
		}
	}
		...
		public void onSelect(Item item) {
			if (item != null) {
				...
				if (ColaDungeon.scene() instanceof GameScene) {
					...
					ColaDungeon.scene().addToFront(new WndEnergizeItem(item, parentWnd));
				}
			}
		}
	};
