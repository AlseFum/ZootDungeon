...
import com.zootdungeon.ui.BadgesGrid;
import com.zootdungeon.ui.BadgesList;
import com.zootdungeon.ui.CustomNoteButton;
import com.zootdungeon.ui.Icons;
import com.zootdungeon.ui.QuickRecipe;
import com.zootdungeon.ui.RedButton;
import com.zootdungeon.ui.RenderedTextBlock;
import com.zootdungeon.ui.ScrollPane;
import com.zootdungeon.ui.ScrollingGridPane;
import com.zootdungeon.ui.ScrollingListPane;
			...
			for (String page : Document.ADVENTURERS_GUIDE.pageNames()){
				...
				ScrollingListPane.ListItem item = new ScrollingListPane.ListItem(
					...
					public boolean onClick(float x, float y) {
						if (inside( x, y ) && found) {
							ColaDungeon.scene().addToFront( new WndStory( Document.ADVENTURERS_GUIDE.pageSprite(page),
		...
		for (Class<?> itemClass : classes) {
			...
			if (Item.class.isAssignableFrom(itemClass)) {
				...
				if (!seen)  {
					...
					if (item.icon != -1) {
						secondIcon = new Image(Assets.getTexture(Assets.Sprites.ITEM_ICONS));
				...
				public boolean onClick(float x, float y) {
					if (inside(x, y)) {
						...
						if (ColaDungeon.scene() instanceof GameScene){
							...
							ColaDungeon.scene().addToFront(new WndJournalItem(sprite, finalTitle, finalDesc));
						}
						return true;
					} else {
						return false;
				...
				public boolean onClick(float x, float y) {
					if (inside(x, y)) {
						...
						if (ColaDungeon.scene() instanceof GameScene){
							...
							ColaDungeon.scene().addToFront(new WndJournalItem(image, finalTitle, finalDesc));
						}
					...
					if (inside(x, y)) {
						...
						if (seen) {
							if (ColaDungeon.scene() instanceof GameScene){
								...
								ColaDungeon.scene().addToFront(new WndStory(sprite, doc.pageTitle(page), doc.pageBody(page)));
							}
							...
							if (ColaDungeon.scene() instanceof GameScene){
								...
								ColaDungeon.scene().addToFront(new WndJournalItem(sprite, "???", Messages.get(CatalogTab.class, "not_seen_lore")));
							}
