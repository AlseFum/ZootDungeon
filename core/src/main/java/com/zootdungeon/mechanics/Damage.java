...
public class Damage {
    ...
    public static PhysicalResult physical(Char attacker, Char defender, float dmgMulti, float dmgBonus, float accMulti, int hitCount) {
        ...
        for (int i = 0; i < hitCount; i++) {
            ...
            // NextAttackDamageBoost is the only external amplifier we still apply inline here.
            base.baseDmg = applyComputeAmplifiers(physicalCtx, base.baseDmg);
    ...
    private static float applyComputeAmplifiers(DamageContext ctx, float amount) {
        ctx.baseAmount = amount;
        if (ctx.from != null && ctx.damageType == PHYSICAL && ctx.damageForm == DIRECT) {
            NextAttackDamageBoost b = ctx.from.buff(NextAttackDamageBoost.class);
            if (b != null) amount *= b.multiplier;
        }
        ctx.baseAmount = amount;
        return amount;
    }

    private static int applyMitigationAmplifiers(DamageContext ctx, int amount) {
        ctx.mitigatedAmount = amount;
        if (ctx.to != null) {
            DefenseBoost def = ctx.to.buff(DefenseBoost.class);
            if (def != null) amount = Math.round(amount * (1f - def.reduction));
        }
        ctx.mitigatedAmount = amount;
        return amount;
    }

    /** 单次伤害应用：防御 proc、DR、黏性、易伤、攻击 proc、音效、扣血、事件分发。返回本段造成的伤害值。 */
    private static int applyOneHit(Char attacker, Char defender, float baseDmg, int dr, boolean visibleFight, boolean playHitSound, int hitIndex, int hitCount) {
        ...
        if (effective >= 0) {
            ...
            effective = Math.max(applyMitigationAmplifiers(hitCtx, effective), 0);
        }
    ...
    private static void postDamageEffects(Char attacker, Char defender, int totalDamage, Preparation prep, PowerStrike nextAttackBoost) {
        ...
        if (gain > 0 && attacker instanceof Hero h) {
            com.zootdungeon.arknights.RhodesIslandTerminal t =
                com.zootdungeon.arknights.RhodesIslandTerminal.ownedBy(h);
            if (t != null) t.addCost(h, gain);
        }
    ...
    // ========== Damage Pipeline (process() path — non-physical damage) ==========

    private static final class DamagePipeline {
        private DamagePipeline() {}

        private static DamageResult applyDamageAndEffects(DamageContext context, int amount, boolean fireEvent) {
            ...
            if (fireEvent) {
                DamageApplied.fire(context, result);
            }
            return result;
        }

        private static DamageResult process(DamageContext context) {
            context.baseAmount = applyComputeAmplifiers(context, context.amount);
            context.mitigatedAmount = applyMitigationAmplifiers(context, Math.round(context.baseAmount));
            int applied = Math.max((int) context.mitigatedAmount, 0);
            return applyDamageAndEffects(context, applied, true);
        }
    }

    /**
     * Fired after damage has been fully computed and applied to the target.
     * At this point the target's HP is already reduced and the {@code result}
     * reflects the final state. Listeners can only read — any attempt to modify
     * values will have no effect on the damage system.
     *
     * <p>Use this for: log messages, particle effects, talent triggers,
     * achievement tracking, audio playback based on damage outcome.
     */
    public static class DamageApplied {
        public final DamageContext context;
        public final DamageResult result;

        private DamageApplied(DamageContext context, DamageResult result) {
            this.context = context;
            this.result = result;
        }

        public static void fire(DamageContext context, DamageResult result) {
            EventBus.dispatchTyped(new DamageApplied(context, result));
        }

        public static void on(java.util.function.Consumer<DamageApplied> handler) {
            EventBus.on(DamageApplied.class, handler);
        }

        public static void on(java.util.function.Consumer<DamageApplied> handler, int priority) {
            EventBus.on(DamageApplied.class, handler, priority);
        }

        public static void off(java.util.function.Consumer<DamageApplied> handler) {
            EventBus.off(DamageApplied.class, handler);
        }
    }
}
