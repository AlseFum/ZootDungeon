...
import com.zootdungeon.actors.buffs.MagicalSight;
...
public class Dungeon {
    ...
    // 旧版本 Dungeon.cost 字段迁移过渡值。Dungeon.restoreFromBundle 读出旧存档里的 cost
    // 暂存到这里，等 RhodesIslandTerminal.restoreFromBundle 取走后清零。新版本不再写回。
    public static int legacyCost;
    ...
    public static void initSeed() {
        if (daily) {
            ...
            seed = CDSettings.lastDaily() + DungeonSeed.TOTAL_SEEDS;
            DateFormat format = new SimpleDateFormat("yyyy-MM-dd", Locale.ROOT);
            format.setTimeZone(TimeZone.getTimeZone("UTC"));
            customSeedText = format.format(new Date(CDSettings.lastDaily()));
        } else if (!CDSettings.customSeed().isEmpty()) {
            customSeedText = CDSettings.customSeed();
    ...
    public static void init() {
        ...
        challenges = CDSettings.challenges();
        mobsToChampion = -1;
    ...
    private static final String LEGACY_COST = "legacy_cost";
    ...
    public static void loadGame(int save, boolean fullLoad) throws IOException {
        ...
        // 旧版本把 cost 存在 "cost" key；本次重构后改存到 RhodesIslandTerminal 上。
        // 读 legacyCost：优先新 key，旧 key 兼容一次。getInt 对缺失 key 返回 0，所以 OR 一下。
        legacyCost = bundle.getInt(LEGACY_COST) | bundle.getInt("cost");
