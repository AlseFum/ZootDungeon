...
package com.zootdungeon.services.news;
...
public class News {
	...
	public static void checkForNews(){
		...
		service.checkForArticles(!CDSettings.WiFi(), useHTTPS, new NewsService.NewsResultCallback() {
			@Override
	...
	public static Image parseArticleIcon(NewsArticle article){
		...
		//if we run into any formatting errors (or icon is null), default to the news icon
			...
			if (article.icon != null) ColaDungeon.reportException(e);
